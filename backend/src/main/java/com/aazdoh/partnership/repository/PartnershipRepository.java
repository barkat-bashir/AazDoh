...
public interface PartnershipRepository extends JpaRepository<AccountabilityPartnership, UUID> {
    ...
    @Query("SELECT p FROM AccountabilityPartnership p WHERE (p.requester.id = :userId OR p.partner.id = :userId) AND p.status = :status")
    List<AccountabilityPartnership> findAllByUserIdAndStatus(@Param("userId") UUID userId, @Param("status") PartnershipStatus status);

    @Query("SELECT p FROM AccountabilityPartnership p WHERE ((p.requester.id = :u1 AND p.partner.id = :u2) OR (p.requester.id = :u2 AND p.partner.id = :u1)) AND p.status <> 'TERMINATED'")
    Optional<AccountabilityPartnership> findActiveBetween(@Param("u1") UUID u1, @Param("u2") UUID u2);

    @Query("SELECT p FROM AccountabilityPartnership p WHERE p.partner.id = :userId AND p.status = 'PENDING'")
    List<AccountabilityPartnership> findPendingIncomingRequests(@Param("userId") UUID userId);

    @Query("SELECT p FROM AccountabilityPartnership p WHERE p.requester.id = :userId AND p.status = 'PENDING'")
